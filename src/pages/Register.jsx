...
const Register = () => {
  ...
  <div className="absolute inset-0 flex flex-col lg:flex-row w-full py-6 px-4 lg:px-10">
    ...
    <div className="w-full lg:w-1/2 flex justify-end">
        ...
        <form className="w-full flex flex-col gap-5 max-w-md bg-white/80 backdrop-blur-md p-6 rounded-xl shadow-lg">
        <h2 className='text-center text-3xl text-blue-700 '>Register</h2>
        <input
          type="email"
...
<div>
      <input type="radio" value="stu" name='categori' />Student

</div>
<div>
      <input type="radio"  value="emp" name='categori'/>Employee
      </div>
      <div>
      <input type="radio" value="Entre" name='categori' />Entrepreneur
      </div>

        </div>
        <button className='p-4 bg-blue-600  cursor-pointer hover:bg-blue-800 rounded-2xl'>Login</button>
        <p className='text-black'>Already have Account <Link to={"/login"} className='text-blue-600 underline'>Login</Link></p>
      </form>
    </div>
  </div>
</div>
